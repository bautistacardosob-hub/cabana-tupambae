  ...
  return (
    ...
    <header className={`siteHeader ${light ? "headerLight" : "headerDark"} ${menuOpen?"menuOpen":""}`}>
      ...
      <nav className="mainNav" aria-label="Navegación principal">
        ...
        <button className={screen === "cabana" ? "active" : ""} onClick={() => go("cabana")}>La cabaña</button>
        <button className={screen === "genetica" || screen === "animal" ? "active" : ""} onClick={() => go("genetica")}>Genética</button>
...
function Home({ go, animals, auctions, siteImages, news, galleryMedia, openAnimal, openNews }: { go: (s: Screen) => void; animals: AnimalRecord[]; auctions: AuctionRecord[]; siteImages:SiteImageMap; news:NewsRecord[]; galleryMedia:GalleryMediaRecord[]; openAnimal: (animal: AnimalRecord) => void; openNews:(post:NewsRecord)=>void }) {
  ...
  return <>
    <section className="hero screenSection" style={{backgroundImage:`url(${siteImages["home-hero"]})`}}>
      ...
      <div className="heroContent"><p className="eyebrow">{content.brand_tagline}</p><h1>{content.home_hero_line_1}<br/><em>{content.home_hero_line_2}</em></h1><p className="heroCopy">{content.home_hero_copy}</p><button className="roundLink" onClick={() => go("genetica")}><span>Explorar<br/>la genética</span><b aria-hidden="true"/></button></div>
      <div className="heroIndex"><span>01</span><i/><span>03</span></div>
    </section>
    {isVisible(content,"show_home_intro")&&<section className="intro" id="cabana"><div className="sectionNumber">01 — La cabaña</div><div>{content.home_intro_eyebrow&&<p className="kicker">{content.home_intro_eyebrow}</p>}<h2>{content.home_intro_line_1}{content.home_intro_line_2&&<><br/><em>{content.home_intro_line_2}</em></>}</h2></div><div className="introCopy">{content.home_intro_copy&&<p>{content.home_intro_copy}</p>}<button className="textLink" onClick={()=>go("cabana")}>Nuestra historia <span>→</span></button></div></section>}
...
function CabinPage({go,siteImages}:{go:(screen:Screen)=>void;siteImages:SiteImageMap}){
  ...
  return <div className="editorialPage"><Header screen="cabana" go={go}/><section className="storyHero"><div>{content.cabana_eyebrow&&<p className="sectionNumber">{content.cabana_eyebrow}</p>}<h1>{content.cabana_title_line_1}{content.cabana_title_line_2&&<><br/><em>{content.cabana_title_line_2}</em></>}</h1></div>{content.cabana_lead&&<p>{content.cabana_lead}</p>}</section><section className="storyPhoto" style={{backgroundImage:`url(${siteImages.establishment})`}}><span>{content.establishment_name} · {content.establishment_location}</span></section><section className="storyBody">{showBrand&&<img className="cabinBrandWatermark" src={siteImages["brand-watermark"]||defaultSiteImages["brand-watermark"]} alt="" aria-hidden="true"/>}<div>{content.cabana_year&&<><small>Desde</small><b>{content.cabana_year}</b></>}</div><article>{content.cabana_story_1&&<p>{content.cabana_story_1}</p>}{content.cabana_story_2&&<p>{content.cabana_story_2}</p>}</article></section>{isVisible(content,"show_history_chapters")&&chapters.length>0&&<section className="historyChapters"><div className="sectionTop"><div><p className="sectionNumber">Historia completa</p><h2>Territorio, selección<br/><em>y producción.</em></h2></div><p>El recorrido de {content.brand_name} desde sus orígenes hasta el trabajo actual.</p></div>{chapters.map(([number,title,copy])=><article key={number}><small>{number}</small><h3>{title}</h3><p>{copy}</p></article>)}</section>}{isVisible(content,"show_story_values")&&<section className="storyValues"><p className="sectionNumber">Nuestra manera de hacer</p><div>{[["01","Datos + campo","Información objetiva acompañada por observación permanente."],["02","Adaptación","Animales funcionales en condiciones reales de producción."],["03","Consistencia","Una dirección genética sostenida generación tras generación."]].map(item=><article key={item[0]}><small>{item[0]}</small><h2>{item[1]}</h2><p>{item[2]}</p></article>)}</div></section>}<Footer go={go}/></div>;
}

function CriollosPage({go,siteImages}:{go:(screen:Screen)=>void;siteImages:SiteImageMap}){
  const content=useSiteContent();
  return <div className="editorialPage criollosPage"><Header screen="criollos" go={go}/><section className="storyHero"><div><p className="sectionNumber">Criollos · {content.brand_name}</p><h1>{content.criollos_title_line_1}<br/><em>{content.criollos_title_line_2}</em></h1></div><p>{content.criollos_lead}</p></section><section className="storyPhoto criollosPhoto" style={{backgroundImage:`url(${siteImages["criollos-hero"]})`}}><span>Caballos Criollos · {content.brand_name}</span></section><section className="criollosBody">{[content.criollos_story_1,content.criollos_story_2,content.criollos_story_3].filter(Boolean).map((copy,index)=><article key={index}><small>{String(index+1).padStart(2,"0")}</small><p>{copy}</p></article>)}</section><Footer go={go}/></div>;
}
...
function Genetics({ go, animals, categories, siteImages, openAnimal }: { go: (s: Screen) => void; animals: AnimalRecord[]; categories:CategoryRecord[]; siteImages:SiteImageMap; openAnimal: (animal: AnimalRecord) => void }) {
  ...
  return <div className="innerPage"><Header screen="genetica" go={go}/>{isVisible(content,"show_genetics_catalog")&&<section className="availableAnimalsBanner"><div><p className="eyebrow">Catálogo actualizado</p><h1>Ver animales<br/><em>disponibles.</em></h1><p>{filtered.length?`${filtered.length} ejemplares publicados con información, imágenes y pedigree.`:"Las nuevas fichas se publicarán próximamente."}</p></div><button onClick={()=>document.getElementById("catalogo-animales")?.scrollIntoView({behavior:"smooth",block:"start"})}>Explorar catálogo <span>↓</span></button></section>}{isVisible(content,"show_genetics_program")&&<><section className="geneticsProgramHero" style={{backgroundImage:`url(${siteImages["genetics-hero"]})`}}><div/><section><p className="eyebrow">Mejoramiento genético</p><h1>Datos, presión de selección<br/><em>y adaptación.</em></h1></section></section><section className="geneticsProgram"><div><p className="sectionNumber">Programa genético</p><h2>{content.genetics_title}</h2></div><p>{content.genetics_copy}</p></section><section className="geneticsStats">{[[content.stat_1_value,content.stat_1_label],[content.stat_2_value,content.stat_2_label],[content.stat_3_value,content.stat_3_label]].filter(([value])=>value).map(([value,label])=><article key={label}><b>{value}</b><span>{label}</span></article>)}</section><section className="geneticsCycle"><div style={{backgroundImage:`url(${siteImages["genetics-cycle"]})`}}/><article><p className="sectionNumber">Resultados</p><h2>{content.genetics_cycle_title}</h2><p>{content.genetics_cycle_copy}</p></article></section></>}{isVisible(content,"show_genetics_catalog")&&<><section className="listingHead" id="catalogo-animales"><p className="sectionNumber">Catálogo genético</p><div><h1>Nuestros<br/><em>animales.</em></h1><p>Las fichas publicadas por la cabaña aparecen automáticamente en este catálogo.</p></div><span className="resultCount">{filtered.length} ejemplares</span></section><section className="filterBar" aria-label="Filtros de animales">{["Todos",...activeCategories.map(item=>item.name)].map(f=><button className={filter===f?"selected":""} onClick={()=>{setFilter(f);setPage(1)}} key={f}>{f}</button>)}</section><section className="catalogTools"><label><span>⌕</span><input aria-label="Buscar en genética" value={query} onChange={e=>{setQuery(e.target.value);setPage(1)}} placeholder="Buscar por nombre, RP, raza o categoría"/></label><select aria-label="Ordenar animales" value={sort} onChange={e=>{setSort(e.target.value as "featured"|"name"|"rp");setPage(1)}}><option value="featured">Destacados primero</option><option value="name">Nombre A–Z</option><option value="rp">RP ascendente</option></select></section>{visible.length?<section className="animalGrid">{visible.map((a,i)=><article className="animalCard" key={a.id??a.name}><button className="cardImage" style={{backgroundImage:`url(${a.image})`}} onClick={()=>openAnimal(a)} aria-label={`Ver ficha de ${a.name}`}><span className="cardIndex">{String((currentPage-1)*pageSize+i+1).padStart(2,"0")}</span>{a.featured&&<span className="featured">Inicio</span>}<span className="openCard">↗</span></button><div className="cardMeta"><div><span>{a.type}</span><h2>{a.name}</h2></div><p>RP {a.rp}<br/>{a.breed}</p></div></article>)}</section>:<section className="catalogEmpty"><span>◇</span><h2>Todavía no hay animales publicados.</h2><p>El programa genético ya está disponible; las fichas se cargarán desde el administrador.</p></section>} {filtered.length>pageSize&&<div className="pagination"><button disabled={currentPage===1} onClick={()=>setPage(value=>Math.max(1,value-1))}>←</button><span>{currentPage} / {pages}</span><button disabled={currentPage===pages} onClick={()=>setPage(value=>Math.min(pages,value+1))}>→</button></div>}</>}<Footer go={go}/></div>;
}
...
function AdminLoading(){return <div className="adminLoading" role="status" aria-live="polite"><div><Brand dark/><i/><p>Cargando administrador…</p></div></div>}
...
function PageContent({siteImages,content,updateSiteImage,updateContent}:{siteImages:SiteImageMap;content:SiteContentMap;updateSiteImage:(image:SiteImageRecord)=>void;updateContent:(values:SiteContentMap)=>void}) {
  ...
  const groups=[
    ...
    {page:"genetica",title:"Programa genético",copy:"Información institucional, indicadores y resultados.",fields:[{key:"genetics_title",label:"Programa · título"},{key:"genetics_copy",label:"Programa · descripción",long:true},{key:"genetics_cycle_title",label:"Resultados · título"},{key:"genetics_cycle_copy",label:"Resultados · texto",long:true},{key:"stat_1_value",label:"Indicador 1 · valor"},{key:"stat_1_label",label:"Indicador 1 · descripción"},{key:"stat_2_value",label:"Indicador 2 · valor"},{key:"stat_2_label",label:"Indicador 2 · descripción"},{key:"stat_3_value",label:"Indicador 3 · valor"},{key:"stat_3_label",label:"Indicador 3 · descripción"}]},
  ...
  const visibilityFields=[
    ...
    {page:"cabana",key:"show_history_chapters",label:"Historia completa",copy:"Capítulos extensos de la cabaña."},{page:"cabana",key:"show_story_values",label:"Valores",copy:"Datos, adaptación y consistencia."},
    {page:"genetica",key:"show_genetics_program",label:"Programa genético",copy:"Introducción, indicadores y resultados."},{page:"genetica",key:"show_genetics_catalog",label:"Catálogo de animales",copy:"Listado y filtros de fichas genéticas."},
    {page:"criollos",key:"show_criollos",label:"Página publicada",copy:"También controla su presencia en el menú."},{page:"actualidad",key:"show_news",label:"Página publicada",copy:"También controla su presencia en el menú."},{page:"galeria",key:"show_gallery",label:"Página publicada",copy:"También controla su presencia en el menú."},{page:"contacto",key:"show_contact",label:"Página publicada",copy:"También controla su presencia en el menú."}
  ];
...
{page:"home",key:"home-hero",label:"Portada principal",copy:"Imagen de apertura",fallback:defaultSiteImages["home-hero"]},{page:"home",key:"establishment",label:"El establecimiento",copy:"Fotografía institucional",fallback:defaultSiteImages.establishment},{page:"genetica",key:"genetics-hero",label:"Programa genético",copy:"Fotografía principal",fallback:defaultSiteImages["genetics-hero"]},{page:"genetica",key:"genetics-cycle",label:"Resultados productivos",copy:"Imagen del bloque de resultados",fallback:defaultSiteImages["genetics-cycle"]},{page:"genetica",key:"animal-gallery-secondary",label:"Galería secundaria",copy:"Respaldo para fichas sin galería",fallback:defaultSiteImages["animal-gallery-secondary"]},{page:"genetica",key:"animal-gallery-tertiary",label:"Galería terciaria",copy:"Tercera imagen de respaldo",fallback:defaultSiteImages["animal-gallery-tertiary"]},{page:"criollos",key:"criollos-hero",label:"Página Criollos",copy:"Imagen protagonista",fallback:defaultSiteImages["criollos-hero"]},{page:"contacto",key:"contact",label:"Página de contacto",copy:"Imagen lateral del formulario",fallback:defaultSiteImages.contact}];
  ...
  const currentGroups=groups.filter(group=>group.page===activePage);const currentVisibility=visibilityFields.filter(field=>field.page===activePage);const currentImages=fields.filter(field=>field.page===activePage);
...
export function SiteApplication({initialScreen="home",initialNewsSlug,initialAnimalId,initialPageSlug,initialPreview=false}:{initialScreen?:Screen;initialNewsSlug?:string;initialAnimalId?:number;initialPageSlug?:string;initialPreview?:boolean}) {
  ...
  useEffect(()=>{
    ...
    const imageRequest=fetch(draftMode?"/api/site-images?draft=1":"/api/site-images").then(async response=>{if(!response.ok)throw new Error("No se pudieron cargar las imágenes del sitio.");const data=await response.json() as {images?:SiteImageRecord[];hasDraft?:boolean};if(data.images?.length)setSiteImages(current=>({...current,...Object.fromEntries(data.images!.map(image=>[image.imageKey,image.url]))}));if(data.hasDraft)setPublicationPending(true)}).catch(()=>setCriticalDataFailed(true));
    const contentRequest=fetch(draftMode?"/api/site-content?draft=1":"/api/site-content").then(async response=>{if(!response.ok)throw new Error("No se pudo cargar la identidad del sitio.");const data=await response.json() as {content?:SiteContentMap;hasDraft?:boolean};if(data.content)setContent(current=>({...current,...data.content}));if(data.hasDraft)setPublicationPending(true)}).catch(()=>setCriticalDataFailed(true));
  ...
  const updateSiteImage=(image:SiteImageRecord)=>{setSiteImages(current=>({...current,[image.imageKey]:image.url}));setPublicationPending(true)};
  const updateContent=(values:SiteContentMap)=>{setContent(current=>({...current,...values}));setPublicationPending(true)};
