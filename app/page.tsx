...
const homeCardImageWidth = 1100;
function animalImageStyle(animal: AnimalRecord, width: number, placement: "default" | "home" = width === homeCardImageWidth ? "home" : "default") {
  const image = readAnimalImagePresentation(animal.image);
  return {
    backgroundImage: `url(${optimizedImageUrl(image.source, width)})`,
    backgroundPosition: `${placement === "home" ? image.homeX : image.x}% ${placement === "home" ? image.homeY : image.y}%`,
    backgroundSize: placement === "home" ? image.homeFit : image.fit,
    backgroundRepeat: "no-repeat",
  };
}

export const defaultSiteImages:SiteImageMap={"brand-logo":"/tupambae-mark-transparent.png","brand-watermark":"/tupambae-mark-transparent.png",...Object.fromEntries(templateSiteImages.map(([key,,url])=>[key,url]))};
...
export const colorPalettes=[
  {id:"tupambae",name:"Tupambaé",copy:"Bordó Hereford, marfil y arena.",colors:["#25221f","#6e2c24","#f2ebdd","#b89b78"]},
...
export function auctionLotItems(value?:string|null){
...
export function Brand({ dark = false }: { dark?: boolean }) {
  ...
  return <div className={`brandMark ${dark ? "brandDark" : "brandLight"}`}><img src={images["brand-logo"]||defaultSiteImages["brand-logo"]} alt="Marca de la cabaña"/><span className="brandWordmark"><small>Cabaña</small><strong>{content.brand_name.replace(/^Cabaña\s+/i,"")}</strong></span></div>;
}
  ...
  return (
    ...
    <header className={`siteHeader ${light ? "headerLight" : "headerDark"} ${menuOpen?"menuOpen":""}`}>
      ...
      <nav className="mainNav" aria-label="Navegación principal">
        ...
        {isVisible(content,"show_genetics")&&<button onClick={() => window.location.assign("/genetica#catalogo-animales")}>Reproductores</button>}
        {nextAuction&&<button className={screen === "remate" ? "active" : ""} onClick={() => go("remate")}>Remates</button>}
      ...
      {menuOpen&&<nav className="mobileNavPanel" aria-label="Navegación móvil"><small>Explorar</small><button onClick={()=>navigate("home")}>Inicio <span>01</span></button>{isVisible(content,"show_cabana")&&<button onClick={()=>navigate("cabana")}>La cabaña <span>02</span></button>}{isVisible(content,"show_genetics")&&<button onClick={()=>navigate("genetica")}>Genética <span>03</span></button>}{isVisible(content,"show_genetics")&&<button onClick={()=>window.location.assign("/genetica#catalogo-animales")}>Reproductores <span>04</span></button>}{nextAuction&&<button onClick={()=>navigate("remate")}>Remates <span>05</span></button>}{isVisible(content,"show_news")&&<button onClick={()=>navigate("actualidad")}>Actualidad <span>06</span></button>}{isVisible(content,"show_gallery")&&<button onClick={()=>navigate("galeria")}>Galería <span>07</span></button>}{isVisible(content,"show_contact")&&<button onClick={()=>navigate("contacto")}>Contacto <span>08</span></button>}{customPages.map((page,index)=><button key={page.id} onClick={()=>window.location.assign(`/p/${page.slug}`)}>{page.menuLabel} <span>{String(index+9).padStart(2,"0")}</span></button>)}<p><Brand/></p></nav>}
    </header>
    <ScrollToTop/>
    </>
  );
...
function Home({ go, animals, auctions, animalsLoaded, auctionsLoaded, siteImages, news, galleryMedia, openAnimal, openNews }: { go: (s: Screen) => void; animals: AnimalRecord[]; auctions: AuctionRecord[]; animalsLoaded:boolean; auctionsLoaded:boolean; siteImages:SiteImageMap; news:NewsRecord[]; galleryMedia:GalleryMediaRecord[]; openAnimal: (animal: AnimalRecord) => void; openNews:(post:NewsRecord)=>void }) {
  const content=useSiteContent();
  ...
  const nextAuctionLots=auctionLotItems(nextAuction?.lots);
  ...
  const auctionSection=nextAuction?<section className="auction auctionFeatured" id="remate"><div className="auctionBackdrop" style={{backgroundImage:`url(${optimizedImageUrl(nextAuction.image,1920)})`}}/><div className="auctionShade"/><div className="auctionLead"><p className="eyebrow">Próximo remate</p><h2>{nextAuction.title.split(" ").slice(0,2).join(" ")}<br/><em>{nextAuction.title.split(" ").slice(2).join(" ")}</em></h2><p className="auctionSummary">{nextAuction.description||nextAuctionLots.join(" · ")}</p></div><div className="auctionFeaturePanel"><div className="auctionDate"><b>{auctionDate?.getDate()||"—"}</b><span>{auctionDate?.toLocaleDateString("es-AR",{month:"long"})||"Fecha"}<br/>{auctionDate?.getFullYear()||"a confirmar"}</span></div><dl>{nextAuction.auctionTime&&<div><dt>Horario</dt><dd>{nextAuction.auctionTime} hs</dd></div>}{nextAuction.auctioneer&&<div><dt>Rematador</dt><dd>{nextAuction.auctioneer}</dd></div>}{nextAuction.location&&<div><dt>Ubicación</dt><dd>{nextAuction.location}</dd></div>}{nextAuctionLots.length>0&&<div><dt>Oferta</dt><dd><ul className="auctionLots">{nextAuctionLots.map((lot,index)=><li key={`${lot}-${index}`}>{lot}</li>)}</ul></dd></div>}</dl><div className="auctionActions"><button onClick={()=>go("remate")}>Ver más información <span>→</span></button>{nextAuction.catalogUrl&&<a href={nextAuction.catalogUrl} target="_blank" rel="noreferrer">Ver catálogo <span>↗</span></a>}{nextAuction.streamUrl&&<a href={nextAuction.streamUrl} target="_blank" rel="noreferrer">Transmisión <span>▶</span></a>}</div></div></section>:<section className="tupAuctionTeaser" id="remate"><div><p className="sectionNumber">03 — Remates</p><h2>El próximo encuentro<br/><em>comienza aquí.</em></h2></div><div><p>La fecha y los detalles del próximo remate se anunciarán cuando estén confirmados por la cabaña.</p><button className="textLink" onClick={()=>go("remate")}>Información de remates <span>↗</span></button></div></section>;
  return <>
    ...
    {auctionPosition==="before_intro"&&auctionsLoaded&&auctionSection}
    ...
    {isVisible(content,"show_home_establishment")&&<section className="widePhoto" style={{backgroundImage:`url(${optimizedImageUrl(siteImages.establishment,1920)})`}} aria-label="Imagen conceptual de ganado Hereford"><div className="photoCaption"><span>{content.establishment_name}</span><span>Imagen conceptual · reemplazar por fotografía de la cabaña</span></div></section>}
    {homeCarousels("before_genetics")}
    {auctionPosition==="before_genetics"&&auctionsLoaded&&auctionSection}
    {isVisible(content,"show_genetics")&&isVisible(content,"show_home_genetics")&&<section className={`geneticsPreview animalCount${homeAnimals.length}`} id="genetica"><div className="previewLead"><p className="sectionNumber">02 — Nuestra genética</p><h2>{content.genetics_line_1}{content.genetics_line_2&&<><br/><em>{content.genetics_line_2}</em></>}</h2><button className="textLink" onClick={() => visibleGenetics.length?window.location.assign("/genetica#catalogo-animales"):go("genetica")}>{visibleGenetics.length?"Ver todos los animales":"Conocer el programa"} <span>↗</span></button></div>{homeAnimals.length===0&&animalsLoaded&&<figure className="tupGeneticsVisual"><img src={siteImages["genetics-hero"]} alt="Toro Hereford, imagen conceptual"/><figcaption>Una mirada Hereford <small>Imagen conceptual · el catálogo se completará con animales reales</small></figcaption></figure>}{homeAnimals.map((a,i)=><article className={`homeAnimal animal${i+1}`} key={a.id??a.name} onClick={() => openAnimal(a)}><div className={`animalImage animalImage-${readAnimalImagePresentation(a.image).fit}`} style={animalImageStyle(a,1100)}>{a.sold&&<span className="soldBadge">Vendido</span>}</div><div><span>{a.type}</span><h3>{a.name}</h3><small>RP {a.rp} · {a.breed}</small></div></article>)}</section>}
    {homeCarousels("before_auction")}
    {auctionPosition==="after_genetics"&&auctionsLoaded&&auctionSection}
    {homeCarousels("after_auction")}
    {auctionPosition==="after_carousels"&&auctionsLoaded&&auctionSection}
    <Footer go={go}/>
  </>;
}
...
function CriollosCatalog({animals,categories,openAnimal}:{animals:AnimalRecord[];categories:CategoryRecord[];openAnimal:(animal:AnimalRecord)=>void}){
  ...
  return <section className="criollosCatalog" id="catalogo-criollos"><section className="listingHead"><p className="sectionNumber">Criollos disponibles</p><div><h1>Criollos<br/><em>en venta.</em></h1><p>Conocé los ejemplares publicados por la cabaña y consultá directamente por cada uno.</p></div><span className="resultCount">{visible.length} ejemplares</span></section>{visible.length>0&&<div className="catalogPdfAction"><a href="/api/animals/pdf?section=criollos" download>↓ Descargar todas las fichas PDF</a></div>}{activeCategories.length>0&&<section className="filterBar" aria-label="Filtros de Criollos">{["Todos",...activeCategories.map(item=>item.name)].map(item=><button key={item} className={filter===item?"selected":""} onClick={()=>setFilter(item)}>{item}</button>)}</section>}<section className="catalogTools"><label><span>⌕</span><input aria-label="Buscar Criollos" value={query} onChange={event=>setQuery(event.target.value)} placeholder="Buscar por nombre, RP, raza o categoría"/></label></section>{visible.length?<AnimalCatalog animals={visible} section="criollos" onOpen={openAnimal}/>:<section className="catalogEmpty"><span>◇</span><h2>No encontramos ejemplares.</h2><p>Probá con otra búsqueda o categoría.</p></section>}</section>;
}
...
function Genetics({ go, animals, categories, siteImages, openAnimal }: { go: (s: Screen) => void; animals: AnimalRecord[]; categories:CategoryRecord[]; siteImages:SiteImageMap; openAnimal: (animal: AnimalRecord) => void }) {
  ...
  return <div className="innerPage"><Header screen="genetica" go={go}/>{isVisible(content,"show_genetics_catalog")&&<section className="availableAnimalsBanner"><div><p className="eyebrow">{filtered.length?"Catálogo Hereford":"Próximamente · catálogo Hereford"}</p><h1>{filtered.length?<>Ver animales<br/><em>disponibles.</em></>:<>Reproductores<br/><em>por presentar.</em></>}</h1><p>{filtered.length?`${filtered.length} ejemplares publicados con información, imágenes y pedigree.`:"Las fichas reales de Tupambaé se incorporarán próximamente."}</p></div><button onClick={()=>document.getElementById("catalogo-animales")?.scrollIntoView({behavior:"smooth",block:"start"})}>Ir al catálogo <span>↓</span></button></section>}{isVisible(content,"show_genetics_program")&&<>{isVisible(content,"show_genetics_hero")&&<section className="geneticsProgramHero" style={{backgroundImage:`url(${optimizedImageUrl(siteImages["genetics-hero"],1920)})`}}><div/><section><p className="eyebrow">Genética Hereford</p><h1>{content.genetics_hero_line_1||"Hereford, una raza."}<br/><em>{content.genetics_hero_line_2||"Muchas posibilidades."}</em></h1></section></section>}{isVisible(content,"show_genetics_intro")&&<section className="geneticsProgram"><div><p className="sectionNumber">Programa genético</p><h2>{content.genetics_title}</h2></div>{content.genetics_copy&&<p>{content.genetics_copy}</p>}</section>}{isVisible(content,"show_genetics_stats")&&stats.length>0&&<section className={`geneticsStats geneticsStats${stats.length}`}>{stats.map(([value,label],index)=><article key={`${label}-${index}`}><b>{value}</b>{label&&<span>{label}</span>}</article>)}</section>}{isVisible(content,"show_genetics_results")&&Boolean(content.genetics_cycle_title||content.genetics_cycle_copy)&&<section className={`geneticsCycle ${showResultsImage?"":"noImage"}`}>{showResultsImage&&<div style={{backgroundImage:`url(${optimizedImageUrl(siteImages["genetics-cycle"],1600)})`}}/>}<article><p className="sectionNumber">Resultados</p>{content.genetics_cycle_title&&<h2>{content.genetics_cycle_title}</h2>}{content.genetics_cycle_copy&&<p>{content.genetics_cycle_copy}</p>}</article></section>}</>}{isVisible(content,"show_genetics_catalog")&&<><section className="listingHead" id="catalogo-animales">{content.genetics_catalog_eyebrow&&<p className="sectionNumber">{content.genetics_catalog_eyebrow}</p>}<div><h1>{content.genetics_catalog_title_line_1}{content.genetics_catalog_title_line_2&&<><br/><em>{content.genetics_catalog_title_line_2}</em></>}</h1>{content.genetics_catalog_copy&&<p>{content.genetics_catalog_copy}</p>}</div><span className="resultCount">{filtered.length} ejemplares</span></section>{filtered.length>0&&<div className="catalogPdfAction"><a href="/api/animals/pdf?section=genetics" download>↓ Descargar todas las fichas PDF</a></div>}<section className="filterBar" aria-label="Filtros de animales">{["Todos",...activeCategories.map(item=>item.name)].map(f=><button className={filter===f?"selected":""} onClick={()=>{setFilter(f);setPage(1)}} key={f}>{f}</button>)}</section><section className="catalogTools"><label><span>⌕</span><input aria-label="Buscar en genética" value={query} onChange={e=>{setQuery(e.target.value);setPage(1)}} placeholder="Buscar por nombre, RP, raza o categoría"/></label><select aria-label="Ordenar animales" value={sort} onChange={e=>{setSort(e.target.value as "featured"|"name"|"rp");setPage(1)}}><option value="featured">Destacados primero</option><option value="name">Nombre A–Z</option><option value="rp">RP ascendente</option></select></section>{visible.length?<AnimalCatalog animals={visible} comparePool={filtered} section="genetics" onOpen={openAnimal} indexOffset={(currentPage-1)*pageSize}/>:<section className="catalogEmpty"><span>◇</span><h2>Las fichas llegarán pronto.</h2><p>El catálogo se completará con los reproductores y datos oficiales de Tupambaé.</p></section>} {filtered.length>pageSize&&<div className="pagination"><button disabled={currentPage===1} onClick={()=>setPage(value=>Math.max(1,value-1))}>←</button><span>{currentPage} / {pages}</span><button disabled={currentPage===pages} onClick={()=>setPage(value=>Math.min(pages,value+1))}>→</button></div>}</>}<Footer go={go}/></div>;
}
...
function ContactPage({go,siteImages}:{go:(screen:Screen)=>void;siteImages:SiteImageMap}) {
  ...
  const canReceiveMessages=Boolean(process.env.NEXT_PUBLIC_SUPABASE_URL);
  return <div className="contactPage"><Header screen="contacto" go={go}/><section className="contactHero"><div><p className="sectionNumber">Contacto · {content.brand_name}</p><h1>{content.contact_line_1}{content.contact_line_2&&<><br/><em>{content.contact_line_2}</em></>}</h1>{content.contact_intro&&<p>{content.contact_intro}</p>}</div><div className="contactDirect"><span>Contacto directo</span>{content.contact_email&&<a href={`mailto:${content.contact_email}`}>{content.contact_email} ↗</a>}{content.contact_whatsapp&&<a className="contactWhatsapp" href={content.contact_whatsapp} target="_blank" rel="noreferrer"><span>Contactar por WhatsApp</span><b>{content.contact_phone||"Abrir conversación"}</b><i>↗</i></a>}{socials.length>0&&<div className="contactSocials">{socials.map(([label,url])=><a key={label} href={url} target="_blank" rel="noreferrer">{label} ↗</a>)}</div>}{content.contact_location&&<small>{content.contact_location}</small>}</div></section><section className={`contactBody ${showContactImage?"":"noImage"}`}>{showContactImage&&<div className="contactPhoto" style={{backgroundImage:`url(${optimizedImageUrl(siteImages.contact,1400)})`}}/>}{canReceiveMessages?<form className="contactForm" onSubmit={e=>{e.preventDefault();void submit(e.currentTarget)}}><div><p className="sectionNumber">Enviar una consulta</p><h2>¿En qué podemos<br/><em>acompañarte?</em></h2></div>{sent?<div className="contactSuccess"><span>✓</span><h3>Consulta recibida.</h3><p>Gracias por escribirnos. El equipo de la cabaña se pondrá en contacto.</p><button type="button" onClick={()=>setSent(false)}>Enviar otra consulta</button></div>:<><div className="contactFields"><label>Nombre y apellido<input name="name" required/></label><label>Email<input name="email" type="email" required/></label><label>Teléfono<input name="phone" type="tel"/></label><label>Motivo<select name="subject"><option>Consulta genética</option><option>Disponibilidad de animales</option><option>Otro</option></select></label><label className="fullField">Mensaje<textarea name="message" required rows={6}/></label></div>{error&&<p className="contactError">{error}</p>}<button className="sendMessage" disabled={busy}>{busy?"Enviando...":"Enviar consulta →"}</button></>}</form>:<div className="tupContactPrompt"><p className="sectionNumber">Contacto inmediato</p><h2>Empecemos una<br/><em>conversación.</em></h2><p>Para esta propuesta, las consultas se reciben directamente por WhatsApp.</p><a href={content.contact_whatsapp} target="_blank" rel="noreferrer">Escribir por WhatsApp ↗</a></div>}</section>{content.contact_map&&<section className="contactMap"><div><p className="sectionNumber">Cómo llegar</p><h2>Ubicación</h2><p>{content.contact_location}</p></div><iframe title={`Mapa de ${content.brand_name}`} src={content.contact_map} loading="lazy" referrerPolicy="no-referrer-when-downgrade"/></section>}<Footer go={go}/></div>;
}
...
function Footer({ go }: { go: (s: Screen) => void }) {const content=useSiteContent();const customPages=customPagesFromContent(content).filter(page=>page.published);const socials=[["WhatsApp",content.contact_whatsapp],["Instagram",content.social_instagram],["Facebook",content.social_facebook],["YouTube",content.social_youtube]].filter((item):item is [string,string]=>Boolean(item[1]?.trim()));return <footer><Brand/><div><p>Genética Hereford<br/>con identidad propia.</p>{content.contact_phone&&<a href={`tel:${content.contact_phone.replace(/\D/g,"")}`}>{content.contact_phone} ↗</a>}{content.contact_email&&<a href={`mailto:${content.contact_email}`}>{content.contact_email} ↗</a>}{socials.length>0&&<div className="footerSocials">{socials.map(([label,url])=><a key={label} href={url} target="_blank" rel="noreferrer">{label} ↗</a>)}</div>}</div><div className="footerLinks">{isVisible(content,"show_cabana")&&<button onClick={()=>go("cabana")}>La cabaña</button>}{isVisible(content,"show_genetics")&&<button onClick={()=>go("genetica")}>Genética</button>}{isVisible(content,"show_genetics")&&<button onClick={()=>window.location.assign("/genetica#catalogo-animales")}>Reproductores</button>}<button onClick={()=>go("remate")}>Remates</button>{isVisible(content,"show_news")&&<button onClick={()=>go("actualidad")}>Actualidad</button>}{isVisible(content,"show_gallery")&&<button onClick={()=>go("galeria")}>Galería</button>}{isVisible(content,"show_contact")&&<button onClick={()=>go("contacto")}>Contacto</button>}{customPages.map(page=><button key={page.id} onClick={()=>window.location.assign(`/p/${page.slug}`)}>{page.menuLabel}</button>)}</div><small>© {new Date().getFullYear()} {content.brand_name} · Imágenes conceptuales hasta incorporar fotografías propias.</small></footer>}
