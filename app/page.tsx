...
import { AnimalCompare } from "./animal-compare";
...
function CriollosCatalog({animals,categories,openAnimal}:{animals:AnimalRecord[];categories:CategoryRecord[];openAnimal:(animal:AnimalRecord)=>void}){
  ...
  return <section className="criollosCatalog" id="catalogo-criollos"><section className="listingHead"><p className="sectionNumber">Criollos disponibles</p><div><h1>Criollos<br/><em>en venta.</em></h1><p>Conocé los ejemplares publicados por la cabaña y consultá directamente por cada uno.</p></div><span className="resultCount">{visible.length} ejemplares</span></section>{activeCategories.length>0&&<section className="filterBar" aria-label="Filtros de Criollos">{["Todos",...activeCategories.map(item=>item.name)].map(item=><button key={item} className={filter===item?"selected":""} onClick={()=>setFilter(item)}>{item}</button>)}</section>}<section className="catalogTools"><label><span>⌕</span><input aria-label="Buscar Criollos" value={query} onChange={event=>setQuery(event.target.value)} placeholder="Buscar por nombre, RP, raza o categoría"/></label></section><AnimalCompare section="criollos"/>{visible.length?<section className="animalGrid">{visible.map((animal,index)=><article className="animalCard" key={animal.id??animal.name}><button className={`cardImage animalImage-${readAnimalImagePresentation(animal.image).fit}`} style={animalImageStyle(animal,1000)} onClick={()=>openAnimal(animal)} aria-label={`Ver ficha de ${animal.name}`}><span className="cardIndex">{String(index+1).padStart(2,"0")}</span>{animal.featured&&<span className="featured">Destacado</span>}{animal.sold&&<span className="soldBadge">Vendido</span>}<span className="openCard">↗</span></button><div className="cardMeta"><div><span>{animal.type}</span><h2>{animal.name}</h2></div><p>{animal.sold&&<><b className="soldText">Vendido</b><br/></>}RP {animal.rp}<br/>{animal.breed}</p></div></article>)}</section>:<section className="catalogEmpty"><span>◇</span><h2>No encontramos ejemplares.</h2><p>Probá con otra búsqueda o categoría.</p></section>}</section>;
}
