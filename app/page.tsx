...
export type CategoryRecord = {id:number;name:string;slug:string;sortOrder:number;active:boolean};
...
function CriollosCatalog({animals,categories,openAnimal}:{animals:AnimalRecord[];categories:CategoryRecord[];openAnimal:(animal:AnimalRecord)=>void}){
  ...
  const activeCategories=categories.filter(item=>item.active&&criollos.some(animal=>animal.type===item.name));
