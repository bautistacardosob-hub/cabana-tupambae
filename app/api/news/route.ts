...
async function valuesFromForm(form:FormData,current?:typeof newsPosts.$inferSelect){
  ...
  let storageKey=current?.storageKey??null;let image=current?.image??"/tupambae-establishment.jpg";const directCover=String(form.get("coverStorageKey")||"").trim();
  if(directCover){if(!directCover.startsWith("news/covers/")||!await bucket().exists(directCover))throw new Error("La portada no terminó de subirse.");if(storageKey&&storageKey!==directCover)await bucket().delete(storageKey);storageKey=directCover;image="/tupambae-establishment.jpg"}
  const file=form.get("file");
  if(file instanceof File&&file.size){if(!allowedImages.has(file.type))throw new Error("Usá una imagen JPG, PNG, WebP o GIF.");if(file.size>maxImageBytes)throw new Error("La imagen no puede superar los 12 MB.");const safeName=file.name.toLowerCase().replace(/[^a-z0-9._-]+/g,"-").slice(-80)||"noticia.jpg";const nextKey=`news/${crypto.randomUUID()}-${safeName}`;await bucket().put(nextKey,file,file.type);if(storageKey)await bucket().delete(storageKey);storageKey=nextKey;image="/tupambae-establishment.jpg"}
