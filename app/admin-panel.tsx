...
function AdminAnimals({openEditor,animals}:{openEditor:(animal:AnimalRecord|null)=>void;animals:AnimalRecord[]}) {
  ...
  return <><section className="adminPageHead"><div><p>Catálogos</p><h1>Animales</h1><span>Gestioná ganado y caballos Criollos por separado.</span></div><div className="adminPageActions"><button className="secondaryAction" onClick={()=>setShowImport(value=>!value)}>Importar Excel</button><button onClick={()=>openEditor(null)}>＋ Agregar animal</button></div></section>{showImport&&<AnimalSpreadsheetImport initialCatalog={catalog} close={()=>setShowImport(false)}/>}<section className="adminPanel animalsPanel"><div className="categoryScope"><button className={catalog==="genetics"?"active":""} onClick={()=>{setCatalog("genetics");setCategory("all");setPage(1)}}>Ganado</button><button className={catalog==="criollos"?"active":""} onClick={()=>{setCatalog("criollos");setCategory("all");setPage(1)}}>Criollos</button></div><div className="tableTools"><label>⌕ <input aria-label="Buscar animales" placeholder="Buscar por nombre, RP, raza..." value={query} onChange={e=>{setQuery(e.target.value);setPage(1)}}/></label><div><button className={status==="all"?"selected":""} onClick={()=>changeStatus("all")}>Todos {catalogAnimals.length}</button><button className={status==="published"?"selected":""} onClick={()=>changeStatus("published")}>Publicados {published}</button><button className={status==="draft"?"selected":""} onClick={()=>changeStatus("draft")}>Borradores {catalogAnimals.length-published}</button></div><select aria-label="Filtrar por categoría" value={category} onChange={e=>{setCategory(e.target.value);setPage(1)}}><option value="all">Todas las categorías</option>{categories.map(item=><option key={item}>{item}</option>)}</select></div>{visible.length?<AnimalRows animals={visible} openEditor={openEditor}/>:<div className="animalSearchEmpty"><span>⌕</span><b>No encontramos animales</b><small>Probá con otra búsqueda o cambiá los filtros.</small></div>}<footer className="tableFooter"><span>Mostrando {visible.length} de {filtered.length} animales</span><div><button disabled={currentPage===1} onClick={()=>setPage(value=>Math.max(1,value-1))}>←</button><b>{currentPage} / {pages}</b><button disabled={currentPage===pages} onClick={()=>setPage(value=>Math.min(pages,value+1))}>→</button></div></footer></section></>
}

type AnimalImportPreview = {
  sourceRow:number; name:string; type:string; rp:string; birthDate:string; registration:string;
  deps:Array<unknown>; errors:string[]; exists:boolean;
};

function AnimalSpreadsheetImport({initialCatalog,close}:{initialCatalog:"genetics"|"criollos";close:()=>void}) {
  const [file,setFile]=useState<File|null>(null);
  const [catalogSection,setCatalogSection]=useState<"genetics"|"criollos">(initialCatalog);
  const [breed,setBreed]=useState(initialCatalog==="criollos"?"Criollo":"Hereford");
  const [status,setStatus]=useState<"draft"|"published">("draft");
  const [mode,setMode]=useState<"skip"|"update">("skip");
  const [preview,setPreview]=useState<AnimalImportPreview[]>([]);
  const [depLabels,setDepLabels]=useState<string[]>([]);
  const [busy,setBusy]=useState(false);
  const [error,setError]=useState("");
  const [message,setMessage]=useState("");
  const request=async(action:"preview"|"import")=>{
    ...
    try{
      const body=new FormData();body.set("file",file);body.set("action",action);body.set("catalogSection",catalogSection);body.set("breed",breed);body.set("status",status);body.set("mode",mode);
      const response=await fetch("/api/animals/import",{method:"POST",body});
      const data=await readApiJson<{preview?:AnimalImportPreview[];depLabels?:string[];created?:number;updated?:number;skipped?:number;invalid?:number;error?:string}>(response);
      if(!response.ok)throw new Error(data.error||"No se pudo procesar el Excel.");
      if(action==="preview"){setPreview(data.preview??[]);setDepLabels(data.depLabels??[]);return}
      setMessage(`✓ ${data.created??0} creados, ${data.updated??0} actualizados y ${data.skipped??0} omitidos.`);
      setTimeout(()=>window.location.reload(),1000);
    }catch(cause){setError(cause instanceof Error?cause.message:"No se pudo procesar el Excel.")}finally{setBusy(false)}
  };
  const valid=preview.filter(item=>!item.errors.length);
  const duplicates=preview.filter(item=>item.exists).length;
  return <section className="adminPanel animalImportPanel"><header><div><small>Carga masiva</small><h2>Importar catálogo desde Excel</h2><p>Revisá las fichas detectadas antes de agregarlas. Las fotos se cargan después en cada animal.</p></div><button type="button" onClick={close} aria-label="Cerrar importador">×</button></header><div className="animalImportSettings"><label>Archivo .xlsx<input type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={event=>{setFile(event.target.files?.[0]??null);setPreview([]);setError("")}}/></label><label>Catálogo<select value={catalogSection} onChange={event=>{const value=event.target.value as "genetics"|"criollos";setCatalogSection(value);setBreed(value==="criollos"?"Criollo":"Hereford");setPreview([])}}><option value="genetics">Genética bovina</option><option value="criollos">Criollos en venta</option></select></label><label>Raza<input value={breed} onChange={event=>{setBreed(event.target.value);setPreview([])}}/></label><label>Estado<select value={status} onChange={event=>setStatus(event.target.value as "draft"|"published")}><option value="draft">Importar como borradores</option><option value="published">Publicar al importar</option></select></label><label>Si el RP ya existe<select value={mode} onChange={event=>setMode(event.target.value as "skip"|"update")}><option value="skip">Omitirlo</option><option value="update">Actualizar datos y DEPs</option></select></label><button type="button" disabled={busy||!file} onClick={()=>void request("preview")}>{busy?"Leyendo...":"Revisar archivo"}</button></div>{preview.length>0&&<><div className="animalImportSummary"><span><b>{preview.length}</b> filas detectadas</span><span><b>{valid.length}</b> listas para importar</span><span><b>{duplicates}</b> RP existentes</span><span><b>{depLabels.length}</b> DEPs reconocidos</span></div><div className="animalImportTable"><div><b>Fila</b><b>Animal</b><b>Categoría</b><b>Datos</b><b>Resultado</b></div>{preview.slice(0,10).map(item=><div key={`${item.sourceRow}-${item.rp}`}><span>{item.sourceRow}</span><span><b>{item.name||"Sin nombre"}</b><small>RP {item.rp||"—"} {item.registration&&`· HBU ${item.registration}`}</small></span><span>{item.type}</span><span>{item.birthDate||"Sin fecha"}<small>{item.deps.length} DEPs</small></span><span className={item.errors.length?"importError":item.exists?"importExisting":"importReady"}>{item.errors.join(" ")||(item.exists?mode==="update"?"Se actualizará":"Se omitirá":"Lista")}</span></div>)}</div>{preview.length>10&&<p className="animalImportMore">Se muestran 10 de {preview.length} filas.</p>}<footer><span>{status==="draft"?"Podrás revisar y publicar cada ficha después.":"Las fichas quedarán visibles inmediatamente."}</span><button type="button" disabled={busy||!valid.length} onClick={()=>void request("import")}>{busy?"Importando...":`Importar ${valid.length} animales`}</button></footer></>}{error&&<p className="editorError">{error}</p>}{message&&<p className="accountSuccess">{message}</p>}</section>;
}
...
function AdminNews({content,updateContent}:{content:SiteContentMap;updateContent:(values:SiteContentMap)=>void}){
  ...
  const fresh:NewsRecord={id:0,title:"",slug:"",excerpt:"",content:"",videoUrl:"",articleImage:null,documentUrl:null,documentFilename:null,category:"Actualidad",image:"/tupambae-establishment.jpg",published:false,updatedAt:""};
  ...
  const submit = async (
    ...
    try {
      await save({
        ...
        breed: String(data.get("breed") || "Hereford"),
        ...
        >
          ...
          <div className="fieldGrid">
            ...
            <label>
              ...
              <input
                name="breed"
                defaultValue={animal?.breed || "Hereford"}
              />
            </label>
            <label>
              Fecha de nacimiento
            ...
            <div className="depsForm">
              ...
              {deps.map((dep, index) => (
                <div className="depsFormRow" key={index}>
                  ...
                  />
                  <select aria-label={`Percentil de ${dep.label || `métrica ${index + 1}`}`}
                  ...
                  >
                    <option value="">Sin percentil</option>
                    {dep.percentile && !["Top 5%", "Top 20%"].includes(dep.percentile) && <option value={dep.percentile}>{dep.percentile} (valor actual)</option>}
...
function PageContent({siteImages,content,updateSiteImage,updateContent}:{siteImages:SiteImageMap;content:SiteContentMap;updateSiteImage:(image:SiteImageRecord)=>void;updateContent:(values:SiteContentMap)=>void}) {
  ...
  const groups=[
    ...
    {page:"genetica",title:"Introducción del programa",copy:"Presentación institucional de la propuesta genética.",visibilityKey:"show_genetics_intro",fields:[{key:"genetics_hero_line_1",label:"Portada · primera línea"},{key:"genetics_hero_line_2",label:"Portada · segunda línea"},{key:"genetics_title",label:"Programa · título"},{key:"genetics_copy",label:"Programa · descripción",long:true}]},
  ...
  const fields=[{page:"general",key:"brand-logo",label:"Marca de la estancia",copy:"Logo visible en todo el sitio",fallback:"/tupambae-mark-transparent.png"},
{page:"general",key:"brand-pdf",label:"Marca para fichas PDF (opcional)",copy:"Si no cargás una imagen, se usa la Marca de la estancia. Subí PNG, WebP o SVG con fondo transparente.",fallback:"/tupambae-mark-transparent.png"},
{page:"general",key:"brand-watermark",label:"Marca ganadera",copy:"Sello de agua para secciones institucionales",fallback:"/tupambae-mark-transparent.png"},
{page:"general",key:"site-icon",label:"Ícono del sitio",copy:"Símbolo cuadrado que aparece en la pestaña del navegador",fallback:"/tupambae-original-mark.png"},
...
{page:"home",key:"home-hero",label:"Portada principal",copy:"Imagen de apertura",fallback:defaultSiteImages["home-hero"]},{page:"home",key:"establishment",label:"El establecimiento",copy:"Fotografía institucional",fallback:defaultSiteImages.establishment},{page:"genetica",key:"genetics-hero",label:"Programa genético",copy:"Fotografía principal",fallback:defaultSiteImages["genetics-hero"]},{page:"genetica",key:"genetics-cycle",label:"Resultados productivos",copy:"Imagen del bloque de resultados",fallback:defaultSiteImages["genetics-cycle"]},{page:"genetica",key:"animal-gallery-secondary",label:"Galería secundaria",copy:"Respaldo para fichas sin galería",fallback:defaultSiteImages["animal-gallery-secondary"]},{page:"genetica",key:"animal-gallery-tertiary",label:"Galería terciaria",copy:"Tercera imagen de respaldo",fallback:defaultSiteImages["animal-gallery-tertiary"]},{page:"criollos",key:"criollos-hero",label:"Página Criollos",copy:"Imagen protagonista",fallback:defaultSiteImages["criollos-hero"]},{page:"contacto",key:"contact",label:"Página de contacto",copy:"Imagen lateral del formulario",fallback:defaultSiteImages.contact}];
  ...
  const resetPdfMark=async()=>{if(!window.confirm("¿Quitar la marca especial del PDF y usar la Marca de la estancia?"))return;setUploading("brand-pdf");setError("");try{const response=await fetch("/api/site-images",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"reset-image",imageKey:"brand-pdf"})});const data=await readApiJson<{image?:SiteImageRecord;error?:string}>(response);if(!response.ok||!data.image)throw new Error(data.error||"No se pudo quitar la marca del PDF.");updateSiteImage(data.image)}catch(cause){setError(cause instanceof Error?cause.message:"No se pudo quitar la marca del PDF.")}finally{setUploading("")}};
