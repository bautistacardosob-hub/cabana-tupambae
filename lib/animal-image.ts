...
export function readAnimalImagePresentation(value?: string | null): AnimalImagePresentation {
  ...
  if (!url) return { source, fit: "cover", x: 50, y: 50 };
  ...
  const cleanSource = url.origin === "https://cabana.local"
    ...
    : url.toString();
  return { source: cleanSource, fit, x, y };
}

export function writeAnimalImagePresentation(source: string, fit: AnimalImageFit, x: number, y: number) {
...
export function resolveAnimalPrimaryImage(value: string | null | undefined, media: AnimalMediaImage[]) {
  ...
  if (current?.url) {
    ...
    return writeAnimalImagePresentation(current.url, presentation.fit, presentation.x, presentation.y);
  }
